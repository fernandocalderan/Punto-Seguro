...
function assignProviders({
  ...
  const eligible = providers
    ...
    .filter((provider) => zoneMatches(lead, provider))
    .filter((provider) => typeMatches(lead, provider))
    .filter((provider) => {
      const assignedToday = leadRepository.countAssignedForProviderOnDate(provider.id, today);
      return assignedToday < provider.daily_cap;
    })
