...
const COLLABORATOR_COMMISSION_TYPES = ["percent", "fixed"];
const COLLABORATOR_STATUSES = ["active", "paused", "banned"];
...
function normalizeCollaboratorCommissionType(value, fallback = "percent") {
  const normalized = String(value || "").trim().toLowerCase();
  if (!normalized) return fallback;
  if (!COLLABORATOR_COMMISSION_TYPES.includes(normalized)) {
    throw new Error("Invalid collaborator commission_type");
  }
  return normalized;
}

function normalizeCollaboratorStatus(value, fallback = "active") {
  const normalized = String(value || "").trim().toLowerCase();
  if (!normalized) return fallback;
  if (!COLLABORATOR_STATUSES.includes(normalized)) {
    throw new Error("Invalid collaborator status");
  }
  return normalized;
}
...
function createProvider(input = {}, existingProvider) {
  ...
  provider.priority = toInteger(input.priority ?? provider.priority, 50);
  provider.daily_cap = Math.max(1, toInteger(input.daily_cap ?? provider.daily_cap, 999));
  provider.last_assigned_at =
    input.last_assigned_at !== undefined
      ? input.last_assigned_at || null
      : provider.last_assigned_at || null;

  if (provider.business_types.length === 1 && provider.business_types[0].toLowerCase() === "all") {
    provider.business_types = [];
  }
...
function createCollaborator(input = {}, existingCollaborator) {
  const nowIso = new Date().toISOString();
  const collaborator = existingCollaborator ? { ...existingCollaborator } : {};

  collaborator.id = collaborator.id || String(input.id || "").trim() || randomUUID();
  collaborator.name = String(input.name ?? collaborator.name ?? "").trim();
  collaborator.type = String(input.type ?? collaborator.type ?? "").trim();
  collaborator.tracking_code = normalizeCollaboratorTrackingCode(input.tracking_code ?? collaborator.tracking_code);
  collaborator.commission_type = normalizeCollaboratorCommissionType(
    input.commission_type ?? collaborator.commission_type ?? "percent",
    "percent"
  );

  const resolvedCommissionValue = Number(input.commission_value ?? collaborator.commission_value ?? 0);
  collaborator.commission_value = Number.isFinite(resolvedCommissionValue) ? Math.max(0, resolvedCommissionValue) : 0;
  collaborator.status = normalizeCollaboratorStatus(input.status ?? collaborator.status ?? "active", "active");
  collaborator.email = String(input.email ?? collaborator.email ?? "").trim().toLowerCase();
  collaborator.phone = String(input.phone ?? collaborator.phone ?? "").trim();
  collaborator.created_at = collaborator.created_at || input.created_at || nowIso;
  collaborator.updated_at = input.updated_at || nowIso;

  if (!collaborator.name) {
    throw new Error("Collaborator name is required");
  }
  if (!collaborator.type) {
    throw new Error("Collaborator type is required");
  }
  if (!collaborator.tracking_code) {
    throw new Error("Collaborator tracking_code is required");
  }
  if (collaborator.commission_value < 0) {
    throw new Error("Collaborator commission_value must be >= 0");
  }
...
function createLead(input = {}, options = {}) {
  ...
  const derivedTicketEstimatedEur = deriveTicketEstimatedEur(leadTypeForPricing, riskLevel);
  const derivedPriceEur = derivePriceEur(leadTypeForPricing, riskLevel);
  const explicitTicketEstimatedEur = Number(input.ticket_estimated_eur);
  const explicitPriceEur = Number(input.price_eur);
  const ticketEstimatedEur = Number.isFinite(explicitTicketEstimatedEur)
    ? Math.max(0, explicitTicketEstimatedEur)
    : derivedTicketEstimatedEur;
  const priceEur = Number.isFinite(explicitPriceEur)
    ? Math.max(0, explicitPriceEur)
    : derivedPriceEur;
  ...
  const collaboratorTrackingCode = normalizeCollaboratorTrackingCode(input.collaborator_tracking_code);
  const collaboratorId = input.collaborator_id ? String(input.collaborator_id).trim() : null;
  const commissionEstimatedRaw = Number(input.commission_estimated_eur);
  const commissionEstimatedEur = Number.isFinite(commissionEstimatedRaw)
    ? Math.max(0, commissionEstimatedRaw)
    : 0;
  ...
  const lead = {
    ...
    collaborator_tracking_code: collaboratorTrackingCode || null,
    commission_estimated_eur: commissionEstimatedEur,
...
function calculateCollaboratorCommission(lead = {}, collaborator = {}) {
  if (!toBoolean(lead.phone_verified, false)) return 0;

  let commissionType = "";
  try {
    commissionType = normalizeCollaboratorCommissionType(
      collaborator.commission_type || collaborator.commissionType || "",
      ""
    );
  } catch (_error) {
    return 0;
  }
  const commissionValueRaw = Number(collaborator.commission_value ?? collaborator.commissionValue);
  const commissionValue = Number.isFinite(commissionValueRaw) ? Math.max(0, commissionValueRaw) : 0;

  if (commissionType === "percent") {
    const priceRaw = Number(lead.price_eur || 0);
