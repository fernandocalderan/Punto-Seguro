...
const { createProvider, createCollaborator, createLead } = require("./models");
...
class CollaboratorRepositoryJson {
  constructor(dataDir) {
    this.repo = new JsonRepository(path.join(dataDir, "collaborators.json"), []);
  }
  ...
  async findByTrackingCode(code) {
    ...
    const collaborators = await this.findAll();
    return collaborators.find((collaborator) => String(collaborator.tracking_code || "").trim().toUpperCase() === trackingCode) || null;
  }
  ...
  async create(input) {
    ...
    const existing = collaborators.find(
      (item) => String(item.tracking_code || "").trim().toUpperCase() === collaborator.tracking_code
    );
    if (existing) {
  ...
  async update(id, patch) {
    ...
    const updated = createCollaborator(patch, collaborators[index]);
    const existing = collaborators.find(
      (item, itemIndex) =>
        itemIndex !== index &&
        String(item.tracking_code || "").trim().toUpperCase() === String(updated.tracking_code || "").trim().toUpperCase()
    );
    if (existing) {
      throw new Error("Collaborator tracking_code already exists");
    }
...
class EvalSnapshotRepositoryJson {
  constructor(dataDir) {
    this.repo = new JsonRepository(path.join(dataDir, "evaluation_snapshots.json"), []);
  }

  async upsert(token, payload, expiresAt) {
    const snapshots = this.repo.read();
    const nowIso = new Date().toISOString();
    const cleanToken = String(token || "").trim();
    const normalizedExpiresAt = new Date(expiresAt || nowIso).toISOString();
    const nextPayload = payload && typeof payload === "object" ? payload : {};

    const index = snapshots.findIndex((item) => item && item.token === cleanToken);
    if (index >= 0) {
  ...
  async findValidByToken(token, nowIso) {
    const cleanToken = String(token || "").trim();
    ...
    const snapshots = this.repo.read();
    const row = snapshots.find((item) => item && item.token === cleanToken) || null;
    if (!row) return null;
  ...
  async delete(token) {
    ...
    const snapshots = this.repo.read();
    const filtered = snapshots.filter((item) => item && item.token !== cleanToken);
    const deleted = filtered.length !== snapshots.length;
    if (deleted) {
      this.repo.write(filtered);
    }
    return deleted;
...
function createJsonRepositories(dataDir) {
  return {
    ...
    collaborators: new CollaboratorRepositoryJson(dataDir),
    events: new EventRepositoryJson(dataDir),
    evalSnapshots: new EvalSnapshotRepositoryJson(dataDir),
  };
}
