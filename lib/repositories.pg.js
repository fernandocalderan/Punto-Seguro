...
class CollaboratorRepositoryPg {
      ...
      for (const seedItem of seedItems) {
        ...
        await query(
          `INSERT INTO collaborators (
             id, name, type, tracking_code, commission_type, commission_value, status, email, phone, created_at, updated_at
           ) VALUES (
  ...
  async findAll() {
    ...
    const result = await query(
      `SELECT id, name, type, tracking_code, commission_type, commission_value, status, email, phone, created_at, updated_at
       FROM collaborators
    ...
    try {
      const result = await query(
        `INSERT INTO collaborators (
         ...
         RETURNING id, name, type, tracking_code, commission_type, commission_value, status, email, phone, created_at, updated_at`,
        [
          collaborator.id,
          collaborator.name,
          collaborator.type,
...
class EvalSnapshotRepositoryPg {
  async upsert(token, payload, expiresAt) {
    const cleanToken = String(token || "").trim();
    const cleanExpiresAt = new Date(expiresAt || Date.now()).toISOString();
    const jsonPayload = payload && typeof payload === "object" ? payload : {};

    await query(
      `INSERT INTO evaluation_snapshots (token, expires_at, payload)
       VALUES ($1, $2::timestamptz, $3::jsonb)
       ON CONFLICT (token) DO UPDATE SET
         payload = EXCLUDED.payload,
         expires_at = EXCLUDED.expires_at,
         updated_at = NOW()`,
      [cleanToken, cleanExpiresAt, JSON.stringify(jsonPayload)]
    );
  }
  ...
  async findValidByToken(token, nowIso) {
    ...
    const result = await query(
      `SELECT token, expires_at, payload
       ...
       LIMIT 1`,
      [cleanToken, new Date(nowIso || Date.now()).toISOString()]
    );
    if (result.rowCount === 0) return null;

    return {
      token: result.rows[0].token,
      expires_at: toIso(result.rows[0].expires_at),
  ...
  async delete(token) {
    ...
    const result = await query(`DELETE FROM evaluation_snapshots WHERE token = $1`, [cleanToken]);
    return result.rowCount > 0;
  }
}
...
function createPostgresRepositories() {
  return {
    ...
    evalSnapshots: new EvalSnapshotRepositoryPg(),
  };
}
